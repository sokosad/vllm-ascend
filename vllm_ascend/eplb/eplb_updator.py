...
class EplbUpdator:
    ...
    def init_eplb(self, expert_map_path, process):
        ...
        self.layer_update_plan_active = False
    ...
    def forward_before(self):
        ...
        if self.get_update_info_flag():
            ...
            self.layer_update_plan_active = bool(self.update_info_all)
        if self.update_expert_weight_flag() and self.layer_update_plan_active:
    ...
    def forward_end(self, eplb_heat_collection_status: bool = True):
        ...
        if (
            not self.global_slots
            and self.layer_update_plan_active
            and self.update_expert_weight_flag()
        ...
        # Global-pool collectives need one clock across real and dummy DP
        # steps. Local heat eligibility controls counters, not this clock.
        if (
            not self.awaiting_plan
            and not (self.global_slots and self.update_plan_active)
            and (
                self.global_slots
                or self.cur_iterations >= self.expert_heat_collection_interval - 1
                or eplb_heat_collection_status
            )
        ):
