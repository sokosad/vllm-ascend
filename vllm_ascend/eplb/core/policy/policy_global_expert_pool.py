...
def _routing_shares(holders: np.ndarray) -> np.ndarray:
    """Replay ascending-holder ``source_rank % copies`` routing.

    Only aggregate logical heat is available, so source ranks are assumed to
    contribute equally. This matches the runtime routing rule, not arbitrary
    source-rank traffic skew. Adding a lower-ranked holder changes the ordinal
    (and potentially the share) of existing copies as well.
    """
    ...
    source_counts = num_ranks // copies + (ordinal < num_ranks % copies)
    return np.where(holders, source_counts / num_ranks, 0.0)
...
def _allocate_critical_path_replicas(
    ...
    contributions = heat[:, :, None] * _routing_shares(holders)
    ...
    for _ in range(slots_per_rank * num_ranks):
        ...
        peak = rank_load.max(axis=-1)[:, None]
        second_moment = np.square(rank_load).sum(axis=-1)[:, None]
        for rank_id in range(num_ranks):
            if remaining[rank_id] == 0:
                continue
            proposed = holders.copy()
            proposed[:, :, rank_id] = True
            candidate_loads = rank_load[:, None, :] - contributions + heat[:, :, None] * _routing_shares(proposed)
            valid = (~holders[:, :, rank_id]) & (heat > 0)
            critical_gain[:, :, rank_id] = np.where(valid, peak - candidate_loads.max(axis=-1), -np.inf)
            second_moment_gain[:, :, rank_id] = np.where(
        ...
        updated = heat[layer_id, expert_id] * _routing_shares(holders[layer_id, expert_id])
...
def _rank_loads_by_layer(table: np.ndarray, heat: np.ndarray, num_ranks: int) -> np.ndarray:
    ...
    return (heat[:, :, None] * _routing_shares(holders)).sum(axis=1)
...
class GlobalExpertPoolPlanner:
    ...
    def plan(self, current_expert_table: Any, expert_workload: Any) -> GlobalExpertPoolDecision:
        ...
        if (
            ...
            pending_loads = _rank_loads_by_layer(self._last_candidate, heat, num_ranks)
            ...
            pending_changes = int(np.count_nonzero(np.any(
                self._last_candidate[:, :, base_slots:] != table[:, :, base_slots:], axis=0
            )))
            pending_cost = (
                _NORMALIZED_MIGRATION_COST * pending_changes
