...
#
# Pure NumPy implementation of the planning portion of CRAFT:
...
def capacity_aware_interleaving(
    ...
    This is Algorithm 3 from CRAFT, applied to total physical experts.  Applying
...
def _replicate_layer(
    ...
    for _ in range(num_replicas):
        ...
        if copy_count[expert] > num_ranks:
            raise RuntimeError(
                "CRAFT replica planner violated the per-expert copy limit"
            )
...
def replay_balancedness(
    ...
    load, _ = _normalise_load(expert_load)
    num_ranks = _strict_integer(num_ranks, "num_ranks", minimum=1)
